...
running each layer's variants on one harness, one corpus, one metric set (Phases 0–15), then
...
CI half-width ±0.10–0.20, so **nothing under ~20 points is resolvable on this eval set.** Read
