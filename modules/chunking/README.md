...
4. **token_f1 is flat (~0.21–0.23) across all chunkers** — the extractive-mock generator can't
   convert better retrieval into a better answer, so answer-quality deltas are invisible until
   a real generator + LLM-judge (Phases 10–11). Retrieval metrics are where chunking shows up.
