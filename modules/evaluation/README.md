...
## Implemented
- `judge.py` — **G-Eval CoT correctness judge** (Claude Haiku/Bedrock, temp 0). Rubric 1–5 →
...
4. **Groundedness INVERTS the ranking — the trap in one row.** The extractive mock scores
   **1.000 groundedness** (it literally copies a context sentence, so it cannot be ungrounded)
   while being only **0.650 correct**. Real Claude paraphrases → 0.80–0.90 groundedness → but
...
5. **`cite_forced` has the highest groundedness of the real styles (0.902 vs 0.796–0.840)** —
